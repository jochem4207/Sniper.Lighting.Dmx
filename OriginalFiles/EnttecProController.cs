...
namespace SniperUsbDmx
{
    public sealed class EnttecProController: IDisplayDmx,IDisposable
    {
        ...
        private Thread writeDMXBUfferThread;
        ...
        {
            return (byte[])bufferToUsb.Clone();
        }

        private bool MoveNextBufferToThread()
        {
            byte[] oldBuffer = GetLiveBuffer();

            byte[] newBuffer = this.nextBuffer;
            this.newData= oldBuffer.CompareBuffers(newBuffer);
           
            if (newData)
            {
                newBuffer.CopyTo(bufferToUsb, 0);

                if (StateChanged != null)
                {
                    StateChanged(null, new StateChangedEventArgs()
                    {
                        CurrentState = bufferToUsb
                    });
                }
            }
            return this.newData;
        ...
        private struct DMXUSBPROParamsType
        {
        ...
        private  bool FTDI_OpenDevice(int device_num, ref FT_STATUS status)
        {
            //return false;
        ...
        */
        ...
        private byte[] GetProHeader(int label, int length)
        {
        ...
        [DllImport("FTD2XX.dll")]
        private static extern unsafe FT_STATUS FT_OpenEx(void* pvArg1, UInt32 dwFlags, ref FT_HANDLE ftHandle);
        [DllImport("FTD2XX.dll")]
        ...
        [DllImport("FTD2XX.dll")]
        private static extern unsafe FT_STATUS FT_ClrRts(FT_HANDLE ftHandle);
        [DllImport("FTD2XX.dll")]
        static extern unsafe FT_STATUS FT_GetQueueStatus(FT_HANDLE ftHandle, ref UInt32 lpdwAmountInRxQueue);
        [DllImport("FTD2XX.dll")]
        private static extern unsafe FT_STATUS FT_GetLatencyTimer(FT_HANDLE ftHandle, ref byte pucTimer);
        [DllImport("FTD2XX.dll")]
        ...
        [DllImport("FTD2XX.dll")]
        private static extern FT_STATUS FT_GetDriverVersion(FT_HANDLE ftHandle, ref uint lpdwVersion);
        [DllImport("FTD2XX.dll")]
        private static extern FT_STATUS FT_SetTimeouts(FT_HANDLE ftHandle, uint ReadTimeout, uint WriteTimeout);
        #endregion
    }
